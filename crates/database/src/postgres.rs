...
CREATE ROLE {runtime} LOGIN PASSWORD {runtime_password} NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION CONNECTION LIMIT 10;
CREATE ROLE {migration} LOGIN PASSWORD {migration_password} NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION CONNECTION LIMIT 2;
CREATE ROLE {backup} LOGIN PASSWORD {backup_password} NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION CONNECTION LIMIT 2;
ALTER ROLE {runtime} SET search_path=app,pg_catalog;
ALTER ROLE {migration} SET search_path=app,pg_catalog;
ALTER ROLE {backup} SET search_path=app,pg_catalog;
